...
# =========================================================
st.set_page_config(
    page_title="Controle de Reclamações de Cliente - V2",
    page_icon="📋",
    layout="wide",
)

BASE_DIR = Path(__file__).resolve().parent
DB_PATH = BASE_DIR / "controle_reclamacoes_v2.db"
UPLOAD_DIR = BASE_DIR / "uploads_v2"
...
def init_db():
    ...
    # migração simples para bases anteriores
...
def importar_ocorrencias_excel(arquivo_excel):
    ...
    for col in df.columns:
        ...
        if chave in ["codigo", "código"]:
            mapa_colunas[col] = "codigo"
        elif chave == "cliente":
            mapa_colunas[col] = "cliente"
        elif chave in ["titulo", "título"]:
            mapa_colunas[col] = "titulo"
        elif chave in ["descricao", "descrição"]:
            mapa_colunas[col] = "descricao"
        elif chave == "quantidade":
            mapa_colunas[col] = "quantidade"
        elif chave in ["data", "data_abertura", "data abertura", "data da ocorrência", "data da ocorrencia"]:
            mapa_colunas[col] = "data_abertura"
        elif chave in ["responsavel_interno", "responsável interno", "responsavel interno"]:
            mapa_colunas[col] = "responsavel_interno"

    df = df.rename(columns=mapa_colunas)

    obrigatorias = ["codigo", "cliente", "titulo"]
    faltantes = [c for c in obrigatorias if c not in df.columns]
    if faltantes:
        return {
            "ok": False,
            "mensagem": "A planilha precisa conter as colunas obrigatórias: código, cliente e título.",
            "incluidas": 0,
            "ignoradas": 0,
        }
    ...
    if "data_abertura" not in df.columns:
        df["data_abertura"] = date.today().isoformat()
    if "responsavel_interno" not in df.columns:
        df["responsavel_interno"] = ""
    ...
    for _, row in df.iterrows():
        ...
        if not codigo or not cliente or not titulo or codigo.lower() == "nan" or cliente.lower() == "nan" or titulo.lower() == "nan":
            ignoradas += 1
            continue

        existente = buscar_ocorrencia_por_codigo(codigo)
        if existente:
            ignoradas += 1
            continue

        data_abertura = pd.to_datetime(row.get("data_abertura"), errors="coerce")
        if pd.isna(data_abertura):
        ...
        responsavel_interno = row.get("responsavel_interno", "")
        responsavel_interno = "" if pd.isna(responsavel_interno) else str(responsavel_interno)
...
# TOPO
# =========================================================
st.title("📋 Controle de Reclamações de Cliente - V2")
st.caption("Cadastro completo no app, consulta por código, edição da ocorrência, semáforo por etapa e dashboard de acompanhamento.")
...
with abas[0]:
    ...
    with st.expander("Up load de planilha Excel", expanded=False):
        st.write("Importe ocorrências em lote para dentro do app.")
        st.write("Colunas obrigatórias: **código**, **cliente** e **título**.")
        st.write("Colunas opcionais: **data_abertura**, **descrição**, **quantidade**, **responsável interno**.")
        ...
        if arquivo_excel is not None and st.button("Importar planilha", key="btn_importar_planilha"):
            ...
            if resultado["ok"]:
                ...
                st.info("Clique em Atualizar para recarregar o painel após o upload.")
            else:
...
# =========================================================
with abas[1]:
    st.subheader("Registro da ocorrência")
    ...
    with st.form("form_ocorrencia", clear_on_submit=True):
        ...
        with col1:
            ...
            data_abertura = st.date_input("Data da ocorrência", value=date.today())
            cliente = st.text_input("Cliente *")
            responsavel_interno = st.text_input("Responsável interno")
        with col2:
            titulo = st.text_input("Título *", placeholder="Ex.: Vazamento em costura lateral")
            quantidade = st.number_input("Quantidade", min_value=0.0, step=1.0, value=0.0)

        descricao = st.text_area("Descrição da ocorrência", height=140)
...
# =========================================================
# ABA 3 - REGISTRAR RETORNO
# =========================================================
with abas[2]:
    st.subheader("Registrar retorno por código")
    ...
    if codigo_busca:
        ...
        if not ocorr:
            ...
            caixa1.markdown(f"**Responsável interno:** {ocorr.get('responsavel_interno') or ''}")
            ...
            caixa3.markdown(f"**Data:** {ocorr['data_abertura']}")
            ...
            semaforo_df = gerar_resumo_semaforo(ocorr)
...
# =========================================================
with abas[3]:
    st.subheader("Consulta e edição da ocorrência")
    st.write("Pesquise pelo código para visualizar a ocorrência, editar dados principais e consultar o histórico.")
    ...
    if codigo_consulta:
        ...
        if not ocorr:
            ...
            i1.write(f"**Data:** {ocorr['data_abertura']}")
            i2.write(f"**Cliente:** {ocorr['cliente']}")
            i2.write(f"**Quantidade:** {ocorr['quantidade']}")
            ...
            st.markdown("### Editar dados da ocorrência")
            with st.form("form_edicao_ocorrencia"):
                e1, e2 = st.columns(2)
                with e1:
                    novo_cliente = st.text_input("Cliente", value=str(ocorr["cliente"] or ""))
                    novo_titulo = st.text_input("Título", value=str(ocorr["titulo"] or ""))
                    novo_responsavel = st.text_input("Responsável interno", value=str(ocorr.get("responsavel_interno") or ""))
                with e2:
                    nova_data = st.date_input(
                        "Data da ocorrência",
                        value=pd.to_datetime(ocorr["data_abertura"]).date() if str(ocorr["data_abertura"]) else date.today()
                    )
                    nova_quantidade = st.number_input("Quantidade", min_value=0.0, step=1.0, value=float(ocorr["quantidade"] or 0.0))
                nova_descricao = st.text_area("Descrição", value=str(ocorr["descricao"] or ""), height=120)
                salvar_edicao = st.form_submit_button("Salvar alterações")

                if salvar_edicao:
                    run_exec("""
                        UPDATE ocorrencias
                        SET cliente = ?, titulo = ?, descricao = ?, quantidade = ?,
                            data_abertura = ?, responsavel_interno = ?, updated_at = ?
                        WHERE id = ?
                    """, [
                        novo_cliente.strip(),
                        novo_titulo.strip(),
...
with abas[4]:
    ...
    if base.empty:
        ...
        if base_dash.empty:
            ...
            with col_d:
                st.markdown("**Ocorrências por responsável interno**")
...
# =========================================================
with abas[5]:
    st.subheader("Exportação do banco")
    ...
    if st.button("Gerar Excel de exportação"):
        ...
        arquivo_saida = BASE_DIR / "exportacao_controle_reclamacoes_v2.xlsx"
        ...
        with open(arquivo_saida, "rb") as f:
            st.download_button(
                label="Baixar Excel",
                data=f,
                file_name="exportacao_controle_reclamacoes_v2.xlsx",
                mime="application/vnd.openxmlformats-officedocument.spreadsheetml.sheet"
            )

st.caption("Versão 2: cadastro completo no app, edição da ocorrência, semáforo por etapa e dashboard com gráficos.")
